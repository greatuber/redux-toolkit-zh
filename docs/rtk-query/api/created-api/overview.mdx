...
在内部，`createApi`将调用[Redux Toolkit的`createSlice` API](https://redux-toolkit.js.org/api/createSlice)来生成一个切片reducer和相应的带有适当逻辑的action创建器，用于缓存获取的数据。它还自动生成一个自定义的Redux中间件，用于管理订阅计数和缓存生命周期。
