...
如果你发现自己想在应用程序的其他地方更新缓存数据，只要你能访问到 `store.dispatch` 方法，包括在 React 组件中通过 [useDispatch](https://react-cn.redux.js.org/api/hooks#usedispatch) 钩子（或者对于 TypeScript 用户，如 [useAppDispatch](https://react-cn.redux.js.org/using-react-redux/usage-with-typescript#define-typed-hooks) 的类型版本）就可以做到。
