...
总结上述行为 - 返回的值必须正确地被记忆化。另请参见[使用选择器派生数据](https://redux.js.org/usage/deriving-data-selectors)和[Redux基础 - RTK查询高级模式](https://redux.js.org/tutorials/essentials/part-8-rtk-query-advanced#selecting-values-from-results)以获取更多信息。
