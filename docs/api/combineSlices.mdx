...
建议从你的 store [推断你的 RootState 类型](https://redux.js.org/usage/usage-with-typescript#define-root-state-and-dispatch-types)，这是从 reducer 推断出来的。然而，如果切片是懒加载的，那么就无法从中推断出来。
