...
Redux 的 [`combineReducers`](https://redux.js.org/api/combinereducers)，为了方便起见重新导出。虽然 `configureStore` 在内部调用了这个，但你可能希望自己调用它来组合多级的 slice reducers。

### `compose`

Redux 的 [`compose`](https://redux.js.org/api/compose)。它从右到左组合函数。
这是一个函数式编程实用工具。你可能想用它来连续应用几个 store 自定义增强器/函数。

### `bindActionCreators`

Redux 的 [`bindActionCreators`](https://redux.js.org/api/bindactioncreators)。它用 `dispatch()` 包装 action 创建者，以便它们在被调用时立即分派。

### `createStore`

Redux 的 [`createStore`](https://redux.js.org/api/createstore)。你不应该直接使用这个。

### `applyMiddleware`

Redux 的 [`applyMiddleware`](https://redux.js.org/api/applymiddleware)。你不应该直接使用这个。
