...
接受一组中间件，并返回一个 [`useDispatch`](https://react-redux.js.org/api/hooks#usedispatch) 钩子，返回一个 `dispatch`，类型化以包含来自提供的中间件的扩展。
...
如果你正在使用 [custom context](https://react-redux.js.org/using-react-redux/accessing-store#providing-custom-context) for React Redux，这将很有用。
