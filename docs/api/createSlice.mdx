...
`createSlice`是一个函数，它接受一个初始状态，一个reducer函数的对象，以及一个"slice名称"，并自动生成与reducers和状态对应的action创建器和action类型。

这个API是编写Redux逻辑的标准方法。

在内部，它使用[`createAction`](./createAction.mdx)和[`createReducer`](./createReducer.mdx)，所以你也可以使用[Immer](../usage/immer-reducers.md)来编写"变异"的不可变更新：
...
`createSlice`接受一个单独的配置对象参数，有以下选项：
