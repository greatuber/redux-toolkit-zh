...
**Redux 文档的 <Link to="https://redux.js.org/usage/deriving-data-selectors">使用 Selectors 派生数据</Link> 页面**涵盖了 selectors 的目的和动机，为什么 memoized selectors 是有用的，典型的 Reselect 使用模式，以及如何与 <ExternalLinks.ReactRedux /> 一起使用 selectors。
